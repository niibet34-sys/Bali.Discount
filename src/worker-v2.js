...
class NotionPromoCleaner {
  element(element) {
    element.append(`<script id="bali-hide-notion-promo">(()=>{
      ...
      const localizeNotionHref = raw => {
        if (!raw) return null;
        try {
          const u = new URL(raw, location.href);
          const host = u.hostname.toLowerCase();
          const notionHost = host === 'balidiscount.notion.site' || host === 'app.notion.com' || host === 'notion.so' || host === 'www.notion.so';
          if (!notionHost) return null;
          const ids = u.pathname.match(/[0-9a-f]{32}/ig);
          if (!ids || !ids.length) return null;
          const id = ids[ids.length - 1];
          return '/ru/' + id + u.search + u.hash;
        } catch { return null; }
      };

      const rewriteNotionLinks = () => {
        document.querySelectorAll('a[href]').forEach(a => {
          const local = localizeNotionHref(a.getAttribute('href'));
          if (local) {
      ...
      document.addEventListener('click', event => {
        ...
        if (!anchor) return;
        const local = localizeNotionHref(anchor.href);
        if (!local) return;
