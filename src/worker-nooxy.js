...
function makeConfig(domain, indexing) {
  return {
    domain,
    ...
    seo: { indexing, brandReplacement: "Bali Discount" },
    nooxy: { showBadge: false },
    customHeadCSS: CUSTOM_HEAD_CSS,
    customBodyJS: CUSTOM_BODY_JS,
  };
}

const previewProxy = initializeNooxy({
  configKey: "bali-discount-workers-preview",
  config: makeConfig("bali-discount.niibet34.workers.dev", false),
});

const productionProxy = initializeNooxy({
  configKey: "bali-discount-production",
  config: makeConfig("bali.discount", true),
});

export default {
  async fetch(request) {
    const url = new URL(request.url);
    const isPreview = url.hostname === "bali-discount.niibet34.workers.dev";
    const proxy = isPreview ? previewProxy : productionProxy;

    if (isPreview && (url.pathname === "/" || url.pathname === "")) {
      return Response.redirect(`${url.origin}/en`, 302);
    }

    const response = await proxy(request);
    const headers = new Headers(response.headers);
    headers.set("x-bali-proxy", isPreview ? "nooxy-2.0.0-preview" : "nooxy-2.0.0-production");
